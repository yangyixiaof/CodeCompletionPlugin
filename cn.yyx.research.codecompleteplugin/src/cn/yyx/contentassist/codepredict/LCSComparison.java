...
public class LCSComparison {
	...
	public static double LCSSimilarityMIs(List<methodInvocationStatement> x, List<methodInvocationStatement> y) {
		...
		Iterator<methodInvocationStatement> iitr = x.iterator();
		int iidx = 0;
		while (iitr.hasNext())
		{
			iidx++;
			methodInvocationStatement ismt = iitr.next();
			Iterator<methodInvocationStatement> jitr = y.iterator();
			int jidx = 0;
			while (jitr.hasNext())
			{
				jidx++;
				methodInvocationStatement jsmt = jitr.next();
				double sim = ismt.Similarity(jsmt);
				if (sim >= PredictMetaInfo.OneSentenceSimilarThreshold)
				{
					c[iidx][jidx] = c[iidx - 1][jidx - 1] + 1;
